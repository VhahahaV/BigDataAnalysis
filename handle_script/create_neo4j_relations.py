...
for node_id1, node1 in nodes.items():
    for node_id2, node2 in nodes.items():
        ...
        # 检查相同任务
        if node1['pipeline_tag'] != 'N/A' and node1['pipeline_tag'] == node2['pipeline_tag']:
        ...
        # 检查相同数据集
        if node1['datasets'] != 'N/A' and node1['datasets'] == node2['datasets']:
            relation_count += 1  # 更新关系计数器
            relations.append({
                ':START_ID': node_id1,
                ':END_ID': node_id2,
        ...
        # 检查相同架构
        if node1['architectures'] != 'N/A' and node1['architectures'] == node2['architectures']:
            relation_count += 1  # 更新关系计数器
            relations.append({
                ':START_ID': node_id1,
                ':END_ID': node_id2,
                ':TYPE': 'same architectures'
            })
